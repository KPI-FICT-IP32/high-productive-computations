...
int main(int argc, char *argv[])
{
    ...
    if (id == root_p) {
        ...
        int cnt = 0;
        float *x = malloc(sizeof(float) * (mx_size - 1));
        for (int p = 1; p < mx_size; p++){
            x[p] = A[p * mx_size - 1];
        }
        ln();

        for (int k = mx_size - 1; k >= 0; k--)
        {
            for (i = k + 1; i < mx_size; i++)
            {
                x[k] = x[k] - A[(k - 1) * mx_size + i - 1] * x[i];
            }
            x[k] = x[k] / (float)A[(k - 1) * mx_size + (k - 1)];
        }

        for (int o = 0; o < mx_size; o++) {
            printf("% *.*f\t", 4, 2, x[o]);
        }
        printf("\n");
